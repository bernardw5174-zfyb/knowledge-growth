...
- **当前版本**：v0.1.2（正式版）。
- **双版本线**：产品包版本（`starter_version`，见 `vault/00-系统/manifest.yaml`）与协议版本（`schema_version`，见 `vault/00-系统/schema.md` 头部）独立演进。产品包版本表示发布形态（当前 v0.1.2）；schema 版本表示协议 / 字段语义修订（当前 0.2.2），可随产品或独立于产品递增。两线数值不一致不是漂移；判冲突以 schema.md 头部声明为准。
